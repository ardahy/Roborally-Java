...
import model.field.ConveyorBelt;
...
/**
 * This class is called when playing Move1, Move2, Move3
 * @author Arda, Ilinur
 */
public class Move1 extends Card {

    String cardName = "Move1";
    ...
    /**
     * moves 1 (2,3) and checks if there is wall or a pit
     * @param player current player
     * @param server used server
     * @author Ilinur
     */
    @Override
    ...
    /**
     *Pushes robots when necessary. As the first step, it determines if there are any robots next to the one that needs
     * to be pushed,then checks for walls that block the push path. Lastly, it pushes any robots that need to be pushed.
     * @param player
     * @param server
     * @author Arda
     */
    public void pushRobots (Player player,Server server){
        //Multiple robots in the same position?
        boolean samePosition = false;
                ...
                while(elementFound2 && !wallFound2){
                    ...
                    for(Player p: server.getPlayingPlayers()){
                        if(p.getRobotX() == lastToPushX2 && p.getRobotY() == lastToPushY2 -1){
                            ...
                            if(server.getWallMap().containsKey(lastToPushX + "_" + kw2)){
                                if(server.getWallMap().get(lastToPushX + "_" + kw2).getOrientation().get(0).equals("bottom")) {
                                    wallFound2 = true;
                                }
                            }
